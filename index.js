...
async function startFaresBot() {
    // استخدام اسم مجلد جديد تماماً لكسر أي حظر سابق
    const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, 'new_fares_session'));
    ...
    const socket = makeWASocket({
        ...
        // تم التغيير لمتصفح سفاري على ماك (أكثر ثقة لواتساب)
...
app.get('/api/pairing', async (req, res) => {
    ...
    try {
        const { state } = await useMultiFileAuthState(path.join(__dirname, 'new_fares_session'));
        ...
        await delay(4000); // زيادة وقت الانتظار لتهيئة الطلب
        const code = await temp.requestPairingCode(num);
        res.json({ status: true, pairing_code: code });
    } catch (e) {
        res.json({ error: "الخدمة مشغولة، جرب بعد قليل" });
    }
});
